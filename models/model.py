...
def get_restaurant_list(stunum):
    sql = "select restaurant from stu3 where stunum = '{stunum}'".format(stunum=stunum)
    result = db.engine.execute(sql)
    return [i[0].replace("餐厅","") for i in result]

def get_place_list(stunum):
    sql = "select place from stu3 where stunum = '{stunum}'".format(stunum=stunum)
    result = db.engine.execute(sql)
